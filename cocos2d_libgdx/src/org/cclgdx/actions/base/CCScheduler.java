...
import org.cclgdx.config.ccConfig;
...
//
/**
 * Scheduler is responsible of triggering the scheduled callbacks. You should
 * not use NSTimer. Instead use this class.
 * 
 * There are 2 different types of callbacks (selectors):
 * 
 * - update selector: the 'update' selector will be called every frame. You can
 * customize the priority. - custom selector: A custom selector will be called
 * every frame, or with a custom interval of time
 * 
 * The 'custom selectors' should be avoided when possible. It is faster, and
 * consumes less memory to use the 'update selector'.
 */
public class CCScheduler {
	...
	private static class tListEntry {
		...
		public CCUpdateCallback callback; // instead of method invocation
	...
	/**
	 * Modifies the time of all scheduled callbacks. You can use this property
	 * to create a 'slow motion' or 'fast fordward' effect. Default is 1.0. To
	 * create a 'slow motion' effect, use values below 1.0. To create a 'fast
	 * fordward' effect, use values higher than 1.0.
	 * 
	 * @since v0.8
	...
	/**
	 * 'tick' the scheduler. You should NEVER call this method, unless you know
	 * what you are doing.
	 */
	public void tick(float dt) {
		...
		for (ConcurrentArrayHashMap<Object, tHashSelectorEntry>.Entry e = hashForSelectors.firstValue(); e != null; e = hashForSelectors.nextValue(e)) {
			...
			// only delete currentTarget if no actions were scheduled during the
			// cycle (issue #481)
	...
	/**
	 * The scheduled method will be called every 'interval' seconds. If paused
	 * is YES, then it won't be called until it is resumed. If 'interval' is 0,
	 * it will be called every frame, but if so, it recommened to use
	 * 'scheduleUpdateForTarget:' instead.
	 * 
	 * @since v0.99.3
	 */
	public void schedule(String selector, Object target, float interval, boolean paused) {
		...
		if (element == null) {
			...
			// Is this the 1st element ? Then set the pause level to all the
			// selectors of this target
	 ...
	 * GC, and obfuscation.
	 */
	public void schedule(CCUpdateCallback callback, Object target, float interval, boolean paused) {
	...
	/**
	 * Unshedules a selector for a given target. If you want to unschedule the
	 * "update", use unscheudleUpdateForTarget.
	 * 
	 * @since v0.99.3
	 */
		...
		if (element != null) {
			for (int i = 0; i < element.timers.size(); i++) {
				...
				if (selector.equals(timer.getSelector())) {
					...
					// update timerIndex in case we are in tick:, looping over
					// the actions
	 ...
	 * GC, and obfuscation.
	 */
	public void unschedule(CCUpdateCallback callback, Object target) {
		// explicity handle nil arguments when removing an object
	...
	/**
	 * Unschedules all selectors from all targets. You should NEVER call this
	...
	/**
	 * Resumes the target. The 'target' will be unpaused, so all schedule
	 * selectors/update will be 'ticked' again. If the target is not present,
	 * nothing happens.
	 */
	...
	/**
	 * Pauses the target. All scheduled selectors/update for a given target
	 * won't be 'ticked' until the target is resumed. If the target is not
	 ...
	 * The 'update' selector will be called every frame. The lower the priority,
	 ...
	 * This is java way version, uses interface based callbacks. UpdateCallback
	 * in this case. It would be preffered solution. It is more polite to Java,
	 * GC, and obfuscation. Target class must implement UpdateCallback or
	 * scheduleUpdate will be used.
	 */
	public void scheduleUpdate(CCUpdateCallback target, int priority, boolean paused) {
		// TODO Auto-generated method stub
		if (ccConfig.COCOS2D_DEBUG >= 1) {
			tHashSelectorEntry hashElement = hashForUpdates.get(target);
			assert hashElement == null : "CCScheduler: You can't re-schedule an 'update' selector'. Unschedule it first";
	...
	// *
	// * @deprecated Use scheduleSelector:forTarget:interval:paused instead.
	// Will
	// * be removed in 1.0
	// */
	// @Deprecated
	...
	//
	// /**
	// * unschedule all timers. You should NEVER call this method, unless you
	// know
	// * what you are doing.
	// *
	...
	public void append(ArrayList<tListEntry> list, Object target, boolean paused) {
		...
		if (target instanceof CCUpdateCallback) {
			listElement.callback = (CCUpdateCallback) target;
		} else {
			try {
